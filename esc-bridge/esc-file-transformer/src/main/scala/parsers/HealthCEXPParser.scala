...
class HealthCEXPParser {

  // =CONCAT("""",SUBSTITUTE(B2," ", ""),"""", " -> ((line: String) => line.substring(", LEFT(E2, FIND("-", E2) -1)*1-1,",", RIGHT(E2, FIND("-", E2) -1)*1,")", "),")

  private val logger = LoggerFactory.getLogger(this.getClass)

  private val healthClaimSchema = new Schema.Parser().parse(GetFileLines("./schema/health-claim.avsc").mkString)
  ...
  positions += ("provider" -> mutable.Map[String, String=>Any](  // changed
    "provider_number" -> ((line: String) => ExtractField(line, Array(1,10))),
    "provider_office" -> ((line: String) => ExtractField(line, Array(10,14))),
    "service_provider_surname" -> ((line: String) => ExtractField(line, Array(14,44))),  // changed
    "service_provider_first_name" -> ((line: String) => ExtractField(line, Array(44,74))),  // changed
  ...
  positions += ("transaction" -> mutable.Map[String, String=>Any](
    ...
    "error_codes" -> ((line: String) => ExtractField(line, Array(393,405))),  // changed
    "esi_messages" -> ((line: String) => ExtractField(line, Array(405,417))),  // changed
    ...
    "number_of_service_codes_on_this_record" -> ((line: String) => ExtractField(line, Array(965,967), toInt)),
    ...
    "services" -> ((line: String) => {
      val arrayElements = mutable.Map[String, (String, Int)=>Any](
        ...
        "service_status" -> ((line: String, offset: Int) => ExtractField(line, Array(1205+offset,1206+offset), {
          case "A" => "Accepted"
          case "R" => "Rejected"
          case other => other
        })),
        ...
        "service_code" -> ((line: String, offset: Int) => ExtractField(line, Array(1224+offset,1229+offset))),
        "service_name_english" -> ((line: String, offset: Int) => ExtractField(line, Array(1249+offset,1284+offset))),
        "service_name_french" -> ((line: String, offset: Int) => ExtractField(line, Array(1284+offset,1319+offset))),
        ...
        // "lab_fee_claimed" -> ((line: String, offset: Int) => ExtractField(line, Array(1361+offset,1367+offset), toLong)),
        // "eligible_amount_lab" -> ((line: String, offset: Int) => ExtractField(line, Array(1367+offset,1373+offset), toLong)),
        // "lab_deductible_amount" -> ((line: String, offset: Int) => ExtractField(line, Array(1373+offset,1379+offset), toLong)),
        // "lab_benefit_amount" -> ((line: String, offset: Int) => ExtractField(line, Array(1379+offset,1385+offset), toLong)),
        // "expense_procedure_code" -> ((line: String, offset: Int) => ExtractField(line, Array(1385+offset,1390+offset))),
        // "expense_claimed" -> ((line: String, offset: Int) => ExtractField(line, Array(1390+offset,1396+offset), toLong)),
        // "expense_eligible_amount" -> ((line: String, offset: Int) => ExtractField(line, Array(1396+offset,1402+offset), toLong)),
        // "expense_deductible_amount" -> ((line: String, offset: Int) => ExtractField(line, Array(1402+offset,1408+offset), toLong)),
        // "expense_benefit_amount" -> ((line: String, offset: Int) => ExtractField(line, Array(1408+offset,1414+offset), toLong)),
        "error_code" -> ((line: String, offset: Int) => ExtractField(line, Array(1414+offset,1426+offset))),  // changed
        "esi_message" -> ((line: String, offset: Int) => ExtractField(line, Array(1426+offset,1438+offset))),  // changed
        ...
        "paid_service_code_1" -> ((line: String, offset: Int) => ExtractField(line, Array(1460+offset,1465+offset))),
        "paid_service_code_2" -> ((line: String, offset: Int) => ExtractField(line, Array(1465+offset,1470+offset))),
        ...
        "carrier_ehc_field" -> ((line: String, offset: Int) => ExtractField(line, Array(1567+offset,1577+offset))),
        "service_code_source" -> ((line: String, offset: Int) => ExtractField(line, Array(1577+offset,1581+offset))),
      ...
      val procedureSchema = healthClaimSchema.getField("transaction").schema().getField("services").schema().getElementType
  ...
  def ParseFile(fileLines: Array[String], result: util.LinkedList[KeyValue[String, GenericRecord]]): Unit = {
    ...
    val issuerSchema = healthClaimSchema.getField("issuer").schema()
    var issuer = new GenericData.Record(issuerSchema)
    val providerSchema = healthClaimSchema.getField("provider").schema().getTypes.get(1)
    var provider = new GenericData.Record(providerSchema)
    val payeeSchema = healthClaimSchema.getField("payee").schema().getTypes.get(1)
    var payee = new GenericData.Record(payeeSchema)
    var providerTransaction = false
    ...
    fileLines.foreach(line => {
      line.substring(0,1).toInt match {
        ...
        case 2 =>
          ...
          provider = CreateGenericRecord(providerSchema, positions, line)
        case 3 =>
          providerTransaction = false
          payee = CreateGenericRecord(payeeSchema, positions, line)
        case 4 | 5 =>
          val transaction = CreateGenericRecord(healthClaimSchema.getField("transaction").schema(), positions, line)
          val claim = new GenericData.Record(healthClaimSchema)
          claim.put("issuer", issuer)
          if (providerTransaction) claim.put("provider", provider)
          ...
          hashedClaims.get(hash) match {
            case Some(_) => logger.info("DUPLICATE HEALTH CLAIM")
            case None =>
