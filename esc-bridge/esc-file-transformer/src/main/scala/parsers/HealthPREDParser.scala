...
class HealthPREDParser {
  ...
  class HealthPredHandler(result: util.LinkedList[KeyValue[String, GenericRecord]]) {
    ...
    positions += ("provider" -> mutable.Map[String, String=>Any](
      "provider_number" -> ((line: String) => ExtractField(line, Array(1,10), toInt)),
      "provider_office" -> ((line: String) => ExtractField(line, Array(10,14))),
      "provider_surname" -> ((line: String) => ExtractField(line, Array(14,44))),
      "provider_first_name" -> ((line: String) => ExtractField(line, Array(44,74))),
    ...
    positions += ("predetermination" -> mutable.Map[String, String=>Any](
      ...
      "provider_number" -> ((line: String) => ExtractField(line, Array(32,41), toInt)),
      ...
      "submission_method" -> ((line: String) => ExtractField(line, Array(291,292), {
        ...
        case "2" => "Manual provider Predetermination"
    ...
    positions += ("detail" -> mutable.Map[String, String=>Any](
      ...
      "service_code" -> ((line: String) => ExtractField(line, Array(22,27), toInt)),
      "service_name_english" -> ((line: String) => ExtractField(line, Array(27,62))),
      "service_name_french" -> ((line: String) => ExtractField(line, Array(62,97))),
      ...
      "paid_service_code_1" -> ((line: String) => ExtractField(line, Array(220,225), toInt)),
      "paid_service_code_2" -> ((line: String) => ExtractField(line, Array(225,230), toInt)),
      "plan_number" -> ((line: String) => ExtractField(line, Array(233,238), toInt)),
      "benefit_code" -> ((line: String) => ExtractField(line, Array(238,243), toInt)),
      ...
      "service_code_source" -> ((line: String) => ExtractField(line, Array(337,341))),
      ...
      "alternate_service_code" -> ((line: String) => ExtractField(line, Array(359,364))),
      "equivalent_service_code" -> ((line: String) => ExtractField(line, Array(364,369))),
    ))

    private val healthPredeterminationSchema = new Schema.Parser().parse(GetFileLines("./schema/health-predetermination.avsc").mkString)
    private val issuerSchema = healthPredeterminationSchema.getField("issuer").schema();
    private var issuer = new GenericData.Record(issuerSchema)
    private val providerSchema = healthPredeterminationSchema.getField("provider").schema().getTypes.get(1)
    private var provider = new GenericData.Record(providerSchema)
    private val clientSchema = healthPredeterminationSchema.getField("client").schema().getTypes.get(1)
    private var client = new GenericData.Record(clientSchema)
    private val predeterminationSchema = healthPredeterminationSchema.getField("predetermination").schema()
    ...
    def SetPredDetails(line: String): Unit = {
      val detail = CreateGenericRecord(predetermination.getSchema.getField("details").schema().getElementType, positions, line)
      details.add(detail)
    }

    def SendToKafka(): Unit = {
      val healthPredetermination = new GenericData.Record(healthPredeterminationSchema)
      healthPredetermination.put("issuer", issuer)
      if (providerTransaction) healthPredetermination.put("provider", provider)
      else healthPredetermination.put("client", client)
      predetermination.put("details", details.clone())
      healthPredetermination.put("predetermination", predetermination)
      val hash = CreateSHA256Hash(healthPredetermination.toString)
      hashedClaims.get(hash) match {
        case Some(_) => logger.info("DUPLICATE HEALTH PRED")
        case None =>
          hashedClaims.put(hash, null)
          result.add(KeyValue.pair(hash, healthPredetermination))
      }
      details.clear()
    }
  ...
  def ParseFile(fileLines: Array[String], result: util.LinkedList[KeyValue[String, GenericRecord]]): Unit = {
    ...
    val healthPredHandler = new HealthPredHandler(result)

    fileLines.foreach(line => {
      line.substring(0,1).toInt match {
        case 0 => healthPredHandler.SetIssuer(line)
        case 2 => healthPredHandler.SetProvider(line)
        case 3 => healthPredHandler.SetClient(line)
        case 4 => healthPredHandler.SetPredGeneral(line)
        case 5 => healthPredHandler.SetPredDetails(line)
        case 6 | 7 => healthPredHandler.SendToKafka()
        case _ =>
      }
    })
  }
